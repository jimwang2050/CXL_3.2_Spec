...
### 2026-06-09 — Ch3 + Ch11 翻译精修报告

**目标**: 校对 Ch3 (CXL 事务层, p.85-190) + Ch11 (CXL 安全, p.892-997) 关键术语一致性, 找未翻译/译法不一致/笔误

**扫描方法**:
- 找所有双语 table row (`<tr><td>EN</td><td style="background-color:#e8e8e8">ZH</td></tr>`)
- 检查 ZH cell: 空/占位/未翻译
- 检查 EN/ZH 长度比
- 抽样式本验证翻译风格

**结果**:
| 章节 | 双语 rows | 空 ZH | 占位 | 真问题 | ZH/EN avg 比 |
|------|-----------|-------|------|--------|------------|
| Ch3  | 330 | 0 | 0 | **0** | 0.46 |
| Ch11 | 166 | 0 | 0 | **0** | 0.41 |

**关键术语统计 (一致性)**:
| 术语 | Ch3 EN/ZH | Ch11 EN/ZH | 评价 |
|------|-----------|-----------|------|
| transaction | 23/43 事务 | 2/1 事务 | 一致 (Ch3 主导) |
| protocol | 13/14 协议 | 59/61 协议 | ✅ 一致 |
| cache | 262/31 缓存 | 135/0 缓存 | ✅ 协议名 CXL.cache 保留为英文 |
| memory | 31/55 内存 | 0/10 内存 | ✅ |
| request | 89/135 请求 | 17/44 请求 | ✅ |
| response | 48/85 响应 | 11/52 响应 | ✅ |
| poison | 4/0 毒化 | 14/17 毒化 | ✅ "Poison" 协议术语保留 |
| IDE | 0/0 | 481/481 IDE | ✅ 协议缩写保留 |

**结论**:
- 翻译质量**已达标**, 无需修改
- 双语项目风格"EN (中文)"得到严格执行: 协议名/缩写 (CXL.cache, IDE, Poison, AES-GCM) 保留英文
- 中英文长度比 0.4-0.5 符合中文压缩特性
- 本次扫描脚本保留在 `~/_work/ch08_fix/translate_audit.py`, 可后续定期复跑
